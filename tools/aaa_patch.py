...
# ---------------------------------------------------------------------------
# V3 DESIGN SYSTEM — deliberately visible, but additive.
# ---------------------------------------------------------------------------
v3_css = r'''
/* =========================================================
...
.brand{
  position:relative;
  padding:10px 13px;
  border:1px solid rgba(184,149,255,.16);
  border-radius:14px;
...
.brand::after{
  ...
  bottom:-7px;
  padding:2px 6px;
  border:1px solid rgba(184,149,255,.20);
  border-radius:999px;
  background:#08070d;
  color:#8876a8;
  font-size:6px;
...
.progress-fill{
  background:linear-gradient(90deg,#5e2bd2,#8a5cff 48%,#d1b7ff 100%);
}

.question-count-chip{
  border-color:rgba(184,149,255,.30);
  background:linear-gradient(180deg,rgba(138,92,255,.11),rgba(138,92,255,.035));
  color:#d5c9e7;
}

.board{
  border-color:rgba(171,126,255,.62);
  border-radius:34px;
...
.naya-orb{
  box-shadow:inset 0 2px 4px rgba(255,255,255,.78),0 0 28px rgba(116,76,255,.48),0 7px 15px rgba(0,0,0,.6);
}

.continue-button:not(:disabled){
  border-color:#c4a4ff;
  box-shadow:inset 0 1px 0 rgba(255,255,255,.12),0 14px 32px rgba(0,0,0,.62),0 0 38px rgba(138,92,255,.22);
}
...
.aaa-status-rail{
  ...
  border-radius:14px;
  background:linear-gradient(90deg,rgba(138,92,255,.075),rgba(255,255,255,.025),rgba(237,66,196,.045));
...
.question-label{
  ...
  box-shadow:0 0 22px rgba(138,92,255,.08);
}
...
.question-title{
  ...
  line-height:1.055;
  text-shadow:0 3px 24px rgba(0,0,0,.55),0 0 40px rgba(138,92,255,.08);
}
...
.answer.selected{
  background:linear-gradient(105deg,rgba(255,255,255,.075),rgba(138,92,255,.045) 55%,rgba(255,255,255,.02));
}
...
@media(max-width:760px){
  ...
  .board-content{padding:15px 9px 13px;}
  .aaa-status-rail{margin-bottom:13px;padding:8px 9px;}
  .question-area{padding:0 5px;}
  .question-label{padding:6px 9px;font-size:8px;}
  .question-title{font-size:clamp(24px,6.5vw,34px);}
  .question-rule{width:130px;margin-top:13px;}
  .answers{width:100%;margin-top:15px;gap:8px;}
  .answer{min-height:63px;grid-template-columns:47px minmax(0,1fr) 27px;gap:9px;padding:6px;border-radius:18px;}
  .jewel{width:45px;height:45px;border-radius:14px;}
  .answer-title{font-size:13.6px;}
  .answer-sub{font-size:12.6px;}
...
# ---------------------------------------------------------------------------
# V3 HUMAN-FACING MICRO COPY / STATUS
# ---------------------------------------------------------------------------
...
# Add a small guidance line below the question rule, without changing the core journey.
...
s, _ = replace_once(s, guidance, guidance)
s, _ = replace_once(s, '  <div\n    class="answers"', '  ' + guidance + '\n\n  <div\n    class="answers"') if 'class="aaa-guidance"' not in s else (s, False)

# ---------------------------------------------------------------------------
# V3 JS: accessible guidance + keyboard answer selection.
# ---------------------------------------------------------------------------
js_css = '''\n.aaa-guidance{margin:12px auto 0;color:#85808f;font-size:11px;line-height:1.4;font-weight:700;letter-spacing:.01em;}\n@media(max-width:760px){.aaa-guidance{font-size:10px;margin-top:9px;padding:0 12px;}}\n'''
s, _ = replace_once(s, '</style>', js_css + '\n</style>')

# Add a compact keyboard shortcut hint to the live guidance after render.
...
# Keyboard navigation among answer buttons.
...
# ---------------------------------------------------------------------------
# Keep the existing compact short-screen optimization exactly once.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Remove accidental duplicate guidance if a previous run partially inserted it.
# ---------------------------------------------------------------------------
needle='<div class="aaa-guidance" id="aaaGuidance" aria-live="polite">Choose the answer that feels most like you. You can change your mind before continuing.</div>'
