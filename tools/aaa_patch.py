...
# 2. Keep exactly one compact short-screen optimization block.
...
# 3. Add the visible AAA status rail once.
...
# 4. Insert the rail immediately before the existing question-area element, regardless of whitespace formatting.
if 'id="aaaStepLabel"' not in s:
    rail_html = '''<div class="aaa-status-rail" aria-live="polite">\n  <span><strong id="aaaStepLabel">STEP 01</strong> · BUILDING YOUR AI PROFILE</span>\n  <span class="aaa-live">ASSESSMENT ACTIVE</span>\n</div>\n\n'''
    pattern = r'(<div\s+class="question-area")'
    s2, n = re.subn(pattern, rail_html + r'\1', s, count=1)
    if n != 1:
        raise RuntimeError('Question area insertion anchor not found.')
    s = s2
...
# 6. Wire to the existing progress state.
