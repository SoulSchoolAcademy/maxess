...
The real assessment payload is authoritative. The repository fixture is only a
standalone-development fallback and is used only when no completed result was
handed to the Results origin.
"""
...
(function(){
  ...
  var params=new URLSearchParams(window.location.search),encoded=params.get('result');
  if(!encoded)return;
  try{
    ...
    if(!payload||payload.version!=='MAXESS-RESULTS-CONTRACT-1'||payload.completed!==true)throw new Error('invalid contract');
  ...
  }catch(e){window.MAXESS_RESULT_ERROR='Invalid MAXESS Result Contract';}
})();
</script>'''
