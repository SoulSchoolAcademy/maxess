#!/usr/bin/env python3
"""Add the growth-oriented scorecard and conversion chapter before NayaNET."""
...
@media(max-width:850px){.mx-growth-grid{grid-template-columns:1fr}.mx-band-rail{grid-template-columns:repeat(2,1fr)}}@media(max-width:620px){.mx-plan{grid-template-columns:1fr}.mx-scorecard-row{grid-template-columns:100px 1fr 38px}.mx-growth-cta{flex-direction:column}.mx-growth-cta .mx-cta{width:100%}}
</style>"""
...
for path in FILES:
    ...
    if 'id="growth-scorecard"' in text:
        continue
    ...
    print(path.name, len(text.splitlines()), len(text.encode()))
